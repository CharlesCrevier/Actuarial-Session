(() => {
  ...
  function frame(t){
    ...
    let g=ctx.createLinearGradient(0,0,0,h); g.addColorStop(0,'#1b1112');g.addColorStop(.45,'#10231d');g.addColorStop(1,'#080405');ctx.fillStyle=g;ctx.fillRect(0,0,w,h);
    const light=ctx.createRadialGradient(w*.68,h*.08,0,w*.68,h*.08,w*.48); light.addColorStop(0,'rgba(255,231,184,.34)');light.addColorStop(.32,'rgba(165,28,48,.12)');light.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=light;ctx.fillRect(0,0,w,h);
    ...
    const shade=ctx.createLinearGradient(0,0,w,0);shade.addColorStop(0,'rgba(10,2,4,.91)');shade.addColorStop(.5,'rgba(18,5,7,.38)');shade.addColorStop(1,'rgba(6,2,3,.28)');ctx.fillStyle=shade;ctx.fillRect(0,0,w,h);
